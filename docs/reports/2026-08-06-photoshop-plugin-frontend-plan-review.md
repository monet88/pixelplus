...
### 11.21 Trạng thái thực thi

- Ngày thực thi: 2026-08-06
- Các đề xuất ở mục 11 đã được áp dụng lên GitHub Issues của `monet88/pixelplus`.

Issue mới được tạo:

| Issue | Nội dung |
| --- | --- |
| #114 | `Plugin P00` foundation: module boundary, commands, manifest, packaging, OpenAPI pipeline |
| #115 | `Plugin P02A` Target Rect mapping + PNG mask codecs |
| #116 | `Plugin P02B` Export Context Rect và capture selection |
| #117 | `Plugin P02C` Place, mask, verify từ local fixture |
| #118 | `Plugin P11A` Variants |
| #119 | `Plugin P11B` Reference images có capability gate |
| #120 | `Plugin P11C` Local data management |
| #121 | `Gateway T28` Từ chối mask non-PNG tại asset ingest |
| #122 | `Gateway T29` Gateway Image Readiness Gate |

Issue được sửa:

| Issue | Thay đổi đã áp dụng |
| --- | --- |
| #95 | Target Rect bất biến, hai loại bounds, ba test seam, ETA hoãn, SecureStorage là cache, licensing ra khỏi scope, dependency graph mới, mở lại multipart |
| #98 | Out-of-scope trỏ tới #121 thay vì "tracked separately" |
| #99 | Thêm bảng quyết định wire contract bắt buộc; bỏ `ready-for-agent` |
| #100 | Snap semantics sửa lại; allowlist chuyển sang #114; blocker đổi thành #114 |
| #101 | Đổi thành capability research cho ChatGPT Codex OAuth cụ thể; bỏ blocker khỏi #109; bỏ `ready-for-agent` |
| #102 | Chuyển thành split parent trỏ #115/#116/#117; bỏ `ready-for-agent` |
| #103 | Bỏ blocker #100, thêm #114, thêm `ConnectionCredentialState` |
| #104 | Thêm bảng quyết định Provider bắt buộc, thêm `FormData` spike; bỏ `ready-for-agent` |
| #105 | ETA đổi thành phase + elapsed; thêm run-state seam tests |
| #106 | Blocker đổi thành #122; thêm generated-client và runtime decoder criteria; bỏ `ready-for-agent` |
| #107 | Thêm placement state contract và placement idempotency |
| #108 | Tách catalogue khỏi runtime binding; blocker đổi sang #114/#99/#103 |
| #109 | Bỏ #101 khỏi blocker; thêm OAuth exchange thật và #111; khóa sign-out semantics; bỏ `ready-for-agent` |
| #110 | Chuyển thành split parent trỏ #118/#119/#120; bỏ `ready-for-agent` |
| #62 | Ghi rõ render đã defer sang #112; thêm #111 làm blocker cho real Transport |
| #70 | Tách basic PR CI ra #123 |

Nhãn `ready-for-agent` hiện chỉ còn ở các ticket đã khóa xong quyết định sản phẩm: #98, #100, #103, #105, #107, #108, #114, #115.

Chưa thực hiện: chuyển các dòng `Blocked by` thành GitHub native issue dependencies. Connector hiện không hỗ trợ mutation quan hệ đó, nên dependency vẫn là Markdown và cần mirror vào Harness story graph.
...
### 12.21 Trạng thái thực thi enforcement

- Ngày thực thi: 2026-08-06
- Các work item ENF-01 đến ENF-07 đã được tạo thành GitHub Issues.

| ID | Issue | Nhãn |
| --- | --- | --- |
| ENF-01 | #123 Add required pull-request CI foundation | `ready-for-agent` |
| ENF-02 | #124 Reconcile the implementation-spec authority fingerprint | `documentation` |
| ENF-03 | #125 Correct sandbox ephemeral and persistent state semantics | `bug` |
| ENF-04 | #126 Add a canonical verify entrypoint and pin the toolchain | `ready-for-agent` |
| ENF-05 | #127 Track a minimal AGENTS.md and fix the architecture authority | `documentation` |
| ENF-06 | #128 Refactor the Public API validator into a library plus CLI | `ready-for-agent` |
| ENF-07 | #129 Add repository governance and dependency-update automation | none |

#124 và #127 cố ý không mang `ready-for-agent`: cả hai cần quyết định authority của con người. #124 đặc biệt nguy hiểm nếu giao cho agent, vì có sẵn một script làm validator xanh lại mà không cần review gì.

#129 không mang nhãn vì bị chặn bởi quyết định license.

Bằng chứng trong mục 12.2 đã được kiểm lại tại thời điểm thực thi và vẫn đúng:

- `.github/workflows/` không có workflow nào được track.
- `gofmt -l apps/gateway` vẫn in ra hơn 10 file tracked.
- `node scripts/validate-provider-gateway-implementation-spec.mjs` vẫn `FAIL: authority content does not match validator-owned fingerprint: CONTEXT.md`.
- `apps/gateway/deploy/sandbox/sandbox.sh:128` vẫn log `no state retained` trong khi named volume `pixelplus-gateway-state` được khai báo persistent tại `docker-compose.yml:77-79`.
- `.gitignore:12-13` vẫn ignore `AGENTS.md` và `CLAUDE.md`.

Lần thực thi này chỉ tạo và sửa GitHub Issues. Không có code, workflow, contract, fingerprint hoặc sandbox behavior nào bị thay đổi.
