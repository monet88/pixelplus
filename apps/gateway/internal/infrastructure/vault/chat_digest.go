...
// inputs: the operation identity, model, and ordered canonical messages.
func (d *HMACChatDigester) CreateFingerprint(
	...
	for _, m := range messages {
		msgs = append(msgs, chatMessagePayload{Role: string(m.Role), Content: m.Content})
	}
	if msgs == nil {
...
func (FailClosedChatDigester) CreateFingerprint(
	domain.ChatOperation, string, []domain.ChatMessage,
) (domain.Fingerprint, error) {
	return "", ports.ErrChatDigesterUnavailable
}
