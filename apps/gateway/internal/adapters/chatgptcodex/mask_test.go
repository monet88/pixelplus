...
// the Codex decision to invert and the local error mapping, each locked so the
// Adapter delegates rather than re-implements.

// TestAlphaMaskFromCanonicalInvertsToAlpha locks the Codex direction of ADR
// 0003 through the per-Adapter wrapper: the wrapper must produce exactly the
// bytes the shared transform produces, proving it delegates and does not
// double-invert or re-implement with drift.
func TestAlphaMaskFromCanonicalInvertsToAlpha(t *testing.T) {
	...
	// The output must round-trip as a PNG with the expected 4x4 bounds.
